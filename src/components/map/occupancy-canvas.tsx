...
function project(
  ...
  const gx = (wx - ox) / resolution;
  const gy = (wy - oy) / resolution;
...
function unproject(
  ...
  return {
    wx: gx * resolution + origin.position.x,
    wy: gy * resolution + origin.position.y,
    gx,
    gy,
  };
}
...
export function OccupancyCanvas({
  ...
  const baked = useMemo(() => {
    ...
    if (bakedRef.current?.src === grid) {
      return bakedRef.current.canvas;
    }
  ...
  useEffect(() => {
    ...
    if (!last || Math.hypot(last.x - pose.x, last.y - pose.y) > 0.05) {
      ...
      if (t.length > trailLength) {
        t.splice(0, t.length - trailLength);
      }
    }
  }, [pose, trailLength]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const poi of pois) {
        ...
        const wx = poi.x * grid.info.resolution + grid.info.origin.position.x;

        const wy = poi.y * grid.info.resolution + grid.info.origin.position.y;
  ...
  const onMouseUp = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (drag.moved || e.button !== 0 || !onMapClick || !grid) {
        return;
      }
